<template>
  <div class="container">
    <h3 class="text-center" style="margin-right: 200px">BFS</h3>
    <div class="row mt-3">
      <div class="col svg">
      </div>
      <div class="col-2">
    ...
    <div class="row mt-3">
      <p>
        <a href="https://en.wikipedia.org/wiki/Breadth-first_search">BFS</a>, or Breadth-first search is one of
        the two elementary graph traversal algorithms. It explores all the neighbor nodes at the present depth
        before moving on to the nodes at the next depth level. This is basically the opposite strategy of its
        counterpart - DFS. BFS uses a
        <a href="https://en.wikipedia.org/wiki/Queue_(abstract_data_type)">Queue</a>. It is useful when
        searching for a shortest path between two nodes in an unweighted graph. It works in O(V + E) time.
      </p>
    </div>
  </div>
</template>
...
export default class DrawAlgorithms extends Vue {
  ...
  async run() {
    ...
    q.offerLast({ id: this.startVertex.id, edgeIndex: -1 });
    const visited = new Set();
    outer:
      for (let level = 0; !q.isEmpty(); level++) {
        for (let size = q.size(); size > 0; size--) {
          const u = q.removeFirst();
          ...
          for (const v of g.get(u.id) || []) {
            ...
            if (!visited.has(v.id)) {
              ...
              visited.add(v.id);
              q.offerLast({ id: v.id, edgeIndex: v.edgeIndex });
            }
          }
        }
      }
